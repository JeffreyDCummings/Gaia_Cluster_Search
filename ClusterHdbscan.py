...
#commonly "0", but it can sometimes be a variety of numbers depending on density of clusters in the field.
extractnum=0
...
clusterfull=pd.read_csv(clustername+"gaiafield2.5.csv",delimiter=",")

#A second dataframe is created which only contains the parameters that will be used to base the HDBSCAN clustering on and analyze the 
...
#A deep copy of the dataframe is created, I then apply several checks to wrap around the RAs together in the case where they fall 
#around RA=0,360.
...
#to a factor of 2.  Especially adjust min_samples if a clear cluster in PM space is missed in the output.  Note that we can also use G 
#magnitude and BP-RP color to increase clustering accuracy.  They are commented out in this input list, but they can help to create a 
#cleaner cluster main sequence, but at the expense of removing many cluster giants/subdwarfs/white dwarfs.
